<%@ Page Language="C#" AutoEventWireup="true" CodeFile="xueshenguanli.aspx.cs" Inherits="xueshenguanli" %>
             ...
             <%------------------功能按钮区第1部分：角色分类---商家/助学管理员----------%>
                 ...
                 <%------------------功能按钮区第2部分：角色分类---商家/助学管理员----------%>
                            <button class="layui-btn layui-btn-danger">
                             ...
                             <button class="layui-btn layui-btn-danger" onclick="delAll(<%# Eval("StuID") %>)">
                 ...
                 <div class="layui-card-body layui-table-body layui-table-main">
                    <asp:Repeater ID="rpTest" runat="server">
                         ...
                         <ItemTemplate>
                           <tr>
                            <td>
                                <input value="<%# Eval("StuID") %>" class="bb" type="checkbox" name="id" lay-skin="primary" > 
                            </td>
                            <td><%# Eval("StuID") %></td>
                            <td><%# Eval("S_Password") %></td>
                            <td><%# Eval("S_name") %></td>
                            <td><%# Eval("S_telnum") %></td>
                            <td><%# Eval("S_Address") %></td>
                            <td><%# Eval("S_Sex") %></td>
                            <td><%# Eval("S_IDCard") %></td>
                            <td><%# Eval("S_Real_Name") %></td>

                            <%---------------------操作列：数据项项操作设置（针对不同角色设置不同操作角色分类提供功能---商家/助学管理员）---------------------%>
                            <td class="td-manage">    
                               <a title="编辑岗位状态" onclick="xadmin.open('修改岗位状态','JobEditState.aspx?id=<%# Eval("StuID") %>',457,650)" href="javascript:;">
                                <i class="layui-icon">&#xe631;</i></a>

                               <a title="修改岗位信息" onclick="xadmin.open('编辑工作岗位信息','EditStudent.aspx?id=<%# Eval("StuID") %>',600,400)"  href="javascript:;">
                                <i class="layui-icon">&#xe643;</i> </a>

                                <a title="删除" onclick="Jobdel(this,'<%# Eval("StuID") %>')" href="javascript:;" class="delete">
                                <i class="layui-icon">&#xe640;</i> </a>

                               <a title="查看岗位详情" onclick="xadmin.open('查看岗位状态','xueshengxiangqing.aspx?id=<%# Eval("StuID") %>',457,650)" href="javascript:;">
                                <i class="layui-icon">&#xe631;</i></a>
 ...
 <script>
     ...
     function Jobdel(obj, id) {
         layer.confirm('确认要删除吗？', function (index) {
             ...
             $.ajax({
                 type: "post",
                 url: "xueshenguanli.aspx/deleteByID",
                 data: "{ID:'" + id + "'}",
